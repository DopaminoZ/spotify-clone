...
function SignUp() {
    ...
    if (formData.email && emailRegex.test(formData.email)) {
      try {
        ...
        if (error.response && error.response.status === 400) {
          setEmailError('An account registered with this email already exists.');
          setEmailValid(false);
        }
      }
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:4000/api/express/signup', {
        username: formData.username,
        email: formData.email,
        password: formData.password,
        dob: new Date(`${formData.dob.year}-${formData.dob.month}-${formData.dob.day}`),
      ...
      <div>
        ...
        <Switch>
          ...
          <Route exact path="/signup/step=3">
            <SignUpS3 formData={formData} onFormDataChange={handleFormDataChange} handleSubmit = {handleSubmit} history={history}/>
          </Route>
          <Route path="/signup">
