...
function LogInPage() {
  ...
  const checkPasswordValid = async () => {
    if (emailError == "Invalid password." && formData.password.length >= 10) {
      try {
        const response = await axios.post(
          "http://localhost:4000/api/express/check-password",
    ...
    <div id={styles.background}>
      <div id={styles.container}>
        ...
        {((!isEmailValid && formData.email) ||
          ...
          >
            <h1 style={{ fontSize: 14, marginLeft: 60, fontWeight: 100 }}>
              {" "}
              {ex} {emailError}
            </h1>
          </div>
        ...
        <div className={styles.buttonscontainer}>
          <button className={styles.buttons}>
            <img className={styles.img1} id={styles.google} src={google} />
            Continue with Google
          </button>
          <button className={styles.buttons}>
            <img className={styles.img1} id={styles.facebook} src={fb} />
            Continue with Facebook
          </button>
          <button className={styles.buttons}>
            <img className={styles.img1} id={styles.apple} src={apple} />
            Continue with Apple
          </button>
          <button className={styles.buttons}>
        ...
        <div className={styles.inputs}>
          <h2 className={styles.text} id={styles.email}>
            Email or username
          </h2>
          <input
            type="text"
          ...
          <div id={styles.inputdiv}>
            ...
            <div id={styles.passbox}>
              ...
              <div onClick={togglePasswordVisibility}>
                {isPasswordVisible ? (
                  <Eyeopen id={styles.eyevisible} />
                ) : (
                  <Eyeclosed id={styles.eyedisable} />
                )}
              </div>
            </div>
          </div>
        </div>
        <button onClick={checkPasswordValid} className={styles.login}>
          Log In
        </button>
